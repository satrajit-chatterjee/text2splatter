...
def main(args):
    ...
    with torch.no_grad():
        gaussian_splat_decoder.conv_in.weight.copy_(vae.decoder.conv_in.weight.repeat(1, 256 // 4, 1, 1))
    ...
    dataset = get_dataset(splatter_cfg, split)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for _, data in enumerate(train_dataloader):
            with accelerator.accumulate(gaussian_splat_decoder):
                ...
                input_images = data["gt_images"][:, :splatter_cfg.data.input_images, ...]
                ...
                pred_gaussian_splats = gaussian_splat_decoder(latents)
            ...
            if accelerator.is_main_process:
                if global_step % args.validation_steps == 0:
                    ...
                    for _ in range(args.num_validation_images):
                        ...
                        validation_input_images = validation_data["gt_images"][:, :splatter_cfg.data.input_images, ...]
                        validation_input_images = validation_input_images.reshape(-1, *validation_input_images.shape[2:])
                        ...
                        val_latents_pred = gaussian_splat_decoder(val_latents)
